...
export interface Messages {
  ...
  tools: {
    ...
    qr: {
      ...
      description: string;
      privacy: string;
      a11y: { help: string; contentType: string };
      preview: {
        prompt: string;
        downloadPng: string;
        downloadSvg: string;
      ...
      /** Formes de modules (clé = square | dots | rounded). */
      shape: Record<string, string>;
      /** Palettes prêtes à l'emploi (clé = classic | ink | …). */
      presets: Record<string, string>;
      /** Niveaux de correction d'erreur (clé = L | M | Q | H). */
      errorCorrection: Record<string, string>;
      /** Messages de validation par règle (clé = url | email | latitude | longitude). */
...
const fr: Messages = {
  ...
  tools: {
    ...
    qr: {
      ...
      description:
        'Génère des QR codes (texte, URL, WiFi, vCard, géo…) personnalisables, en local. Export PNG/SVG.',
      privacy:
        "Généré localement dans votre navigateur — aucune donnée n'est envoyée à un serveur.",
      a11y: { help: 'Aide', contentType: 'Type de contenu' },
      preview: {
      ...
      shape: { square: 'Carré', dots: 'Points', rounded: 'Arrondi' },
      presets: {
        classic: 'Classique',
        ink: 'Encre',
        ocean: 'Océan',
      ...
      types: {
        ...
        sms: {
          label: 'SMS',
          description: 'Ouvre l’application de messages avec un SMS pré-rempli.',
        },
...
const en: Messages = {
  ...
  tools: {
    ...
    qr: {
      ...
      description:
        'Generate customizable QR codes (text, URL, WiFi, vCard, geo…) locally. PNG/SVG export.',
      privacy: 'Generated locally in your browser — no data is sent to any server.',
      a11y: { help: 'Help', contentType: 'Content type' },
      preview: {
      ...
      shape: { square: 'Square', dots: 'Dots', rounded: 'Rounded' },
      presets: {
        classic: 'Classic',
        ink: 'Ink',
        ocean: 'Ocean',
      ...
      types: {
        ...
        url: {
          label: 'URL',
          description: 'A web link. `https://` is added automatically if missing.',
        },
